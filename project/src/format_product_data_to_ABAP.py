...
def get_year_range(year_min, year_max, golden_df):
    ...
    valid_years = golden_df[
        # Check if the golden data year range overlaps with our target range
        ((golden_df["Year Min"].fillna(0).astype(int) <= year_max) & 
         (golden_df["Year Max"].fillna(0).astype(int) >= year_min))
    ]["Year Min"].fillna(0).astype(int).unique()  # Ensure years are integers
...
def expand_all_models(year, make, golden_df):
    ...
    models = golden_df[
        (golden_df["Year Min"].fillna(0).astype(int) <= year) & 
        (golden_df["Year Max"].fillna(0).astype(int) >= year) &
    ...
    for _, row in product_df.iterrows():
        ...
        for year in years:
            if model == "ALL":
                ...
                for m in models:
                    car_ids.append(f"{int(year)}_{make}_{m}")
            else:
                car_ids.append(f"{int(year)}_{make}_{model}")
