    ...
    def ai_match_models_for_year_make(
        ...
        try:
            ...
            if result.confidence < confidence_threshold:
                print(f"⚠️  AI confidence {result.confidence} below threshold {confidence_threshold}, using fallback")
                return self._fallback_fuzzy_model_match(year_make_matches, input_model)
            ...
            if len(selected_matches) == 0:
                print("⚠️  AI selected no valid car_ids, using fallback")
                return self._fallback_fuzzy_model_match(year_make_matches, input_model)
...
RESPONSE: Just the make name or "NO_MATCH" (no explanation needed)"""
    ...
    def _create_model_matching_prompt(
        ...
        prompt = f"""Match vehicle to car_ids:

INPUT: {' | '.join(input_specs)}
OPTIONS: {' | '.join(options_text)}

Select ALL appropriate matching car_ids. Consider model similarity, body type, doors.

JSON response:
{{
    "selected_car_ids": ["car_id1", "car_id2", "car_id3"],
    "confidence": 0.85,
    "match_type": "ai_model_match"
}}

Rules: Use exact car_ids from options. Confidence 0.0-1.0. Select as many as appropriate."""
    ...
    def _fallback_fuzzy_model_match(self, year_make_matches: pd.DataFrame, input_model: str) -> pd.DataFrame:
        ...
        for idx, row in year_make_matches.iterrows():
            ...
            if similarity >= 0.6:  # Same threshold as original
                similarities.append({
                    'idx': idx,
                    'similarity': similarity,
                    'car_id': row['car_id'],
        ...
        if similarities:
            ...
            print(f"⚠️  No fuzzy model matches found above threshold 0.6")
            return pd.DataFrame()